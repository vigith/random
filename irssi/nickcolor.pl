...
$VERSION = "1";
%IRSSI = (
    authors     => "Timo Sirainen, Ian Peters, Vigith Maurice",
    contact	=> "tss\@iki.fi", 
    name        => "Nick Color",
    description => "assign a different color for each nick",
    license	=> "Public Domain",
    url		=> "http://irssi.org/",
    changed	=> "2013-12-03T19:47+0000"
);
...
sub sig_public {
  ...
  my $rev = '';
  $rev = chr(22) if ($color > 13);
  $server->command('/^format pubmsg {pubmsgnick $2 {pubnick '.$rev.chr(3).$color.'$0}}$1');
}
...
sub cmd_color {
  ...
  if (!$op) {
    ...
    } elsif ($color < 2 || $color > 28) {
      Irssi::print ("Color must be between 2 and 28 inclusive ('/color preview' will list all the colors)");
    } else {
      $saved_colors{$nick} = $color;
    }
  } elsif ($op eq "clear") {
    ...
    foreach my $nick (keys %saved_colors) {
      my $rev = '';
      $rev = chr(22) if ($saved_colors{$nick} > 13);
      Irssi::print ($rev.chr (3) . "$saved_colors{$nick}$nick" .
		    $rev.chr (3) . "1 ($saved_colors{$nick})");
    }
  } elsif ($op eq "preview") {
    Irssi::print ("\nAvailable colors:");
    foreach my $i (2..28) {
      my $rev = '';
      $rev = chr(22) if ($i > 13);
      Irssi::print ($rev.chr (3) . "$i" . "Color #$i");
    }
  }
}
